...
const readJSON = file => () => fs.readFile(path.resolve(__dirname, 'data', file), 'utf8')
  .then(data => JSON.parse(data))

const writeJSON = file => data => fs.writeFile(path.resolve(__dirname, 'data', file), JSON.stringify(data, null, 2), {
  encoding: 'utf8'
})
    ...
    fields: () => ({
      ...
      book: {
        ...
        resolve(parent, { title, id }) {
          return data.books.read()
          .then(R.find(book => book.id === id || book.title.toLowerCase().match(title)))
        }
      },
      authors: {
      ...
      author: {
        ...
        resolve(parent, { name, id }) {
          return data.authors.read()
          .then(R.find(author => author.id === id || author.name.toLowerCase().match(name) )
          )
        }
      },
    })
  ...
  mutation: new GraphQLObjectType({
    ...
    fields: () => ({
      addAuthor: {
        ...
        resolve(parent, { name }) {
          return data.authors.create({ name })
            .then(trace('mutation'))
        }
      },
      /*
